# Take a CSV export of our VAULT Subjects & Genres sheet
# https://docs.google.com/spreadsheets/d/1la_wsFPOkHLjpv4-f3tWwMsCd0_xzuqZ5xp_p1zAAoA/edit#gid=1465207925
# and convert it into several files for Invenio migration:
# - subjects_map.json: a mapping of VAULT subject strings to Invenio subject IDs
# - one YAML file per subject scheme (form, name, place, topic)
#
# Usage:
# python migrate/mk_subjects.py subjects.csv
# Run from the project root like this. It takes additional static vocabularies we made  in the
# "vocabs" dir and includes them. The YAML files are written to the vocab directory and the JSON
# file is written to the migrate directory. YAML files are copied to the Invenio repo if the
# INVENIO_REPO environment variable is set.
import csv
import json
import os
import sys
import uuid
from pathlib import Path

import yaml


def get_uuid(term: str) -> str:
    # This doesn't make sense but is consistent across runs so it'll do
    return str(uuid.uuid5(uuid.NAMESPACE_URL, term))


def dump_all(subjects_map, vocabs: dict[str, list[dict[str, str]]]) -> None:
    with open("migrate/subjects_map.json", "w") as file:
        json.dump(subjects_map, file, indent=2)
    for name, data in vocabs.items():
        with open(f"vocab/subject_{name}.yaml", "w") as file:
            yaml.dump(data, file)
    # if we have INVENIO_REPO, copy output files to the repo's vocab dir
    if "INVENIO_REPO" in os.environ:
        dest: Path = Path(os.environ["INVENIO_REPO"]) / "app_data" / "vocabularies"
        if dest.exists():
            for name, data in vocabs.items():
                with open(dest / f"subject_{name}.yaml", "w") as file:
                    yaml.dump(data, file)


def main(file: str) -> None:
    with open(file, "r") as fp:
        subjects_map: dict[str, str] = {}
        vocabs: dict[str, list[dict[str, str]]] = {
            "form": [],
            "name": [],
            "place": [],
            "topic": [],
        }

        reader = csv.DictReader(fp)
        for row in reader:
            auth: str = row["Auth"].upper()
            status: str = row["Status"].lower()  # omit, combine, done, problem
            term: str = row["New Value"] if row["New Value"] else row["VAULT value"]
            subject: dict[str, str] = {"subject": term}

            if status in ("omit", "problem", ""):
                continue

            if auth == "LOCAL":
                # Combined local subjects _must_ have a new value
                if status == "combine":
                    if not row["New Value"]:
                        raise ValueError(
                            f"Combined local subject without a New Value: {term}"
                        )
                subject["id"] = get_uuid(term)
            else:
                # All external terms use their Auth URI as the ID
                if not row["Auth URI"]:
                    raise ValueError(
                        f"No Auth URI for Getty/Wikidata term: {term}\nExternal terms must have an Auth URI."
                    )
                subject["id"] = row["Auth URI"]

            subject["scheme"] = row["type"].lower()
            if subject["scheme"] not in vocabs.keys():
                raise ValueError(
                    f"Unknown subject scheme '{subject['scheme']}' for term: {term}"
                )

            # ! a limitation of this scheme-less term:id mapping is we cannot have the same term
            # ! in multiple vocabs (e.g., a term that is both a form and a topic)
            subjects_map[row["VAULT value"].lower()] = subject["id"]
            # combined terms are not added to the final vocab files (but are in the map)
            if status == "done":
                vocabs[subject["scheme"]].append(subject)

        # add premade sub-vocabs (only names for now)
        for filename in ["vault_names.yaml"]:
            with open(Path("vocab") / filename, "r") as fh:
                terms: list[dict[str, str]] = yaml.load(fh, Loader=yaml.FullLoader)
                for t in terms:
                    # if term has already been added to the subjects_map, skip it
                    if (term_text := t["subject"].lower()) in subjects_map:
                        continue
                    # assign an ID that matches what we have in the map from combined subjects.csv terms
                    t["id"] = get_uuid(t["subject"])
                    subjects_map[term_text] = t["id"]
                    vocabs[t["scheme"]].append(t)

        dump_all(subjects_map, vocabs)


if __name__ == "__main__":
    main(sys.argv[1])
